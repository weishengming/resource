...
if [ ! $env ]; then  
    ...
    JAVA_OPT="-Xmx2048m -Xms1024m -Xmn512m -Xss512k -XX:PermSize=256m -XX:SurvivorRatio=8 -XX:ParallelGCThreads=20 -XX:ParallelCMSThreads=20 -XX:+UseConcMarkSweepGC -XX:+UseCMSCompactAtFullCollection -XX:CMSFullGCsBeforeCompaction=5 -XX:CMSInitiatingOccupancyFraction=70 -XX:+PrintClassHistogram"
fi
...
start() {
    ...
    su $SERVICE_USER -c "nohup $SPRINGBOOTAPP_JAVA $JAVA_OPT -jar $SPRINGBOOTAPP_WAR --spring.config.location=file:${CONFIG}/application.properties --logging.config=file:$CONFIG/logback.xml  >> $LOG 2>&1 &"    
    echo $cnt
    echo "line : $cnt "
